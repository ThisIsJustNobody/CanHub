...
当前发布范围包含帧模型、注册表与 DI 基础设施、进程内虚拟适配器、Vector XL Driver 适配器、ZLG USBCANFD 适配器，以及 Vector ASC Trace 导入/导出。示例项目和硬件探测工具暂不纳入第一阶段发布范围。
...
| [CanHub.Trace.VectorAsc](src/CanHub.Trace.VectorAsc/README.zh-CN.md) | 不依赖硬件的 Vector ASC Trace 读取/写入器 | MIT |
...
`CanHub.Trace.VectorAsc` 可以导入和导出常见 Vector 风格 `.asc` CAN Trace 文件：
...
大型文件可使用 `VectorAscReader.ReadFileFrames(path)` 逐行流式读取帧。

首版支持 Classic CAN 数据帧、远程帧、错误帧，以及带 BRS/ESI/DLC/data length 的 CAN FD 数据帧。LIN、信号解码、总线统计和厂商专用状态行默认跳过，并通过诊断暴露。
...
CanHub.Abstractions、CanHub.Core、CanHub.Adapter.Virtual 和 CanHub.Trace.VectorAsc 使用 MIT License。
